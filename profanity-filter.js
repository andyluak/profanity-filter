...
const fuzzyWordCheck = (text, word) => {
...
const checkStopWords = (unformattedText, UNFILTERED_STOP_WORDS, RELAXED_STOP_WORDS) => {
    ...
    const replacedText = unformattedText
        ...
        if (substitutionMapValues.some((subs) => subs.includes(char))) {
            const key = Object.keys(substitutionMap).find((k) => substitutionMap[k].indexOf(char) > -1);
            return key;
        }
        return char;
    })
